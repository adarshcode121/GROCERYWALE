...
// Check if child_id is set, otherwise redirect to first child category
if ($child = $childs->fetch_assoc()) {
    if (!isset($_GET['child_id']) || !is_numeric($_GET['child_id'])) {
        header("Location: products.php?parent_id=$parent_id&child_id=" . $child['id']);
        exit();
    }
}
?>
