...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Convert offer percentage to numeric value
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $image = $item['image']; // Keep old image if no new one is uploaded
    }

    // **Fix Bind Parameter Order and Types**
    $stmt = $conn->prepare("UPDATE items SET name=?, description=?, price=?, old_price=?, offer=?, weight=?, category_id=?, image=? WHERE id=?");
    $stmt->bind_param("ssddssisi", $name, $description, $price, $old_price, $offer, $weightsString, $category_id, $image, $id);
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>Offer:</label>
                <input type="text" name="offer" value="<?= $item['offer'] ?>">
            </div>
